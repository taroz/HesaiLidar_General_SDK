...
int main(int argc, char** argv) {
  std::string lidarType = "PandarXTM";
  std::string fileName = lidarType + ".csv";
  ...
  PandarGeneralSDK pandarGeneral(std::string("192.168.1.201"), 2368, 0, 10110, \
      lidarCallback, lidarAlgorithmCallback, gpsCallback, 0, 0, 1, lidarType, std::string("frame_id"), "", "", "", false);

  pandarGeneral.Start();

  // wait for load calibration data
  sleep(1);
  
  std::cout << pandarGeneral.GetLidarCalibration() << std::endl;
  ofs << pandarGeneral.GetLidarCalibration() << std::endl;
